...
export default function BlogRows() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      const res = await axios.get('https://apriltwins-backend.onrender.com/api/blogs');
    ...
    <section className="max-w-screen-xl m-auto px-4 py-6">
              ...
              <div key={idx} className="flex flex-col gap-[10px]">
                {row.map((blog) => (
                  <div key={blog._id} className="bg-white h-[700px] shadow rounded overflow-hidden">
                    <div
                      style={{ backgroundImage: `url(${blog.media})` }}
      ...
      {selectedBlog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center px-4 ">
            ...
            <div className="h-[300px] bg-cover bg-center" style={{ backgroundImage: `url(${selectedBlog.media})` }}></div>
